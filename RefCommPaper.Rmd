...
---
### Abstract
Children have considerable difficulty producing informative and unambiguous referring expressions, a fact that still lacks a full explanation. Models from adult psycholinguistics suggest that speakers avoid ambiguity by pro-actively monitoring for potential ambiguity in any to-be-described scene, as well as subsequently monitoring their own descriptions to check if they are ambiguous. Across three experiments, we show that eye tracking can be used to measure both pro-active and subsequent monitoring of ambiguity in adults, and then use that finding to assess the developing roles of these skills in children's referential communication. Our data suggest that children typically do not pro-actively monitor for potential ambiguity but, when they do show evidence of monitoring, that they tend to subsequently produce informative expressions. Meanwhile, we find consistent evidence that children can monitor their own descriptions and subsequently avoid producing the same ambiguous expression. We propose that the process of subsequent monitoring might act as a learning signal, that guides how children learn to monitor pro-actively. 
<br>
<br>
...
We used the exact same materials and procedure as in the nonlinguistic ambiguity condition of Experiment 1, except that we replaced the instruction phrase with XXXX. 
...
`r round(subset(Labels.Expt3, condition == "ambiguous" & Response == "foil")$Label.mean,2) ` 
 ...
 ). Our regression analysis showed that participants were reliably more likely to produce specific descriptions when the trial was ambiguous `r format_coef(Expt3.Verbal.Fixed, 2)`, and that this interacted with the type of picture being labeled, such that the effect of trial type was reliably larger for the foil picture than the target picture `r format_coef(Expt3.Verbal.Fixed, 4)`. Although this was not a large effect, it is consistent with production monitoring. 
