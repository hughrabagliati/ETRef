...
output:
  ...
  word_document:
    reference: APA.docx
csl: apa.csl
abstract: Children have considerable difficulty producing informative and unambiguous referring expressions, a fact that still lacks a full explanation. Models from adult psycholinguistics suggest that speakers avoid ambiguity by pro-actively monitoring for potential ambiguity in any to-be-described scene, as well as subsequently monitoring their own descriptions to check if they are ambiguous. Here, we used eye tracking to  
bibliography: 
  - ETRefLibrary.bib
references:
...
**Methods**  
*Participants*   
`r length(unique(Sac.sum$Subj))` undergraduates from the University of Edinburgh.
...
**Figure 2. Proportion of saccades between target picture and foil in the Linguistic Ambiguity condition, across time windows.***
```{r, echo=FALSE, results='hide',message=FALSE}
# Same Cat
SacGraph(na.omit(summaryBy(SacTarg~Start+cond+Subj, data = Sac.sum[ Sac.sum$type == "Same",], FUN = c(mean,sd), keep.names = T)))
```
   
**Figure 3. Proportion of saccades between target picture and foil in the Non-Linguistic Ambiguity condition, across time windows.***   
...
Children produced more referentiall specific descriptions of the Target picture when the scene was ambiguous than when it was unambiguous
(Mean<sub>ambiguous</sub>=
